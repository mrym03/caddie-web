/*
 * Liquid-glass effect — ported from paper.design's "liquid metal" shader
 * (web-visuals/liquid-logo). Renders a beveled height-field image (precomputed
 * from the Caddie monogram) as flowing liquid chrome. WebGL2; one canvas per
 * .liquid-canvas element, lazily started when scrolled into view.
 */
(function () {
  'use strict';
  ...
  // Fragment shader — verbatim from liquid-logo/src/app/hero/liquid-frag.ts
...
void main() {
  ...
  var PARAMS = { patternScale: 2, refraction: 0.015, edge: 0.4, patternBlur: 0.005, liquid: 0.07, speed: 0.26 };
  ...
  function mount(canvas) {
    ...
    for (var i = 0; i < nU; i++) { var nm = gl.getActiveUniform(prog, i).name; U[nm] = gl.getUniformLocation(prog, nm); }
    ...
    function resize() {
      ...
      var side = Math.round((canvas.clientWidth || 480) * dpr);
      if (side < 2) return;
    ...
    // texture from the precomputed height-field image
    ...
    // placeholder 1x1 so it renders before the image loads
    ...
    img.src = canvas.getAttribute('data-field');

    var visible = false, last = 0, time = 0, raf = 0;
    function frame(now) {
  ...
  function boot() {
    var nodes = document.querySelectorAll('.liquid-canvas');
